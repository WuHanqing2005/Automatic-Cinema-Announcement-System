...
version_code = '2025.03.07 23:00(正式版)'     # 版本号
...
# 清空缓存的函数，清除所有output文件夹下合成的音频文件，并返回布尔值
def delete_all_files_in_directory(directory_path):
    ...
    try:
        ...
        # 清空储存已播放电影的列表film_played
        global film_played
        film_played = []
        print('film_played has been cleared.')
...
def fetch_movie_schedules(url, progress_window):
    ...
    try:
        ...
        def sort_key(row):
            ...
            # 解析日期详情
            if date_day:
                month, day = date_day.split('月')
                day = day.rstrip('日')
                # 将月份和日期转换为整数
                month = int(month)
                day = int(day)
                # 返回一个元组 (月份, 日期, 开始时间)
                return (month, day, start_time)
            else:
                # 如果日期详情为空，返回一个默认值
